...
def predict():
    try:
        ...
        cursor.execute('''
            INSERT INTO health_data (age, gender, body_temperature, pulse_rate, respiration_rate, 
            blood_pressure, blood_oxygen, weight, blood_glucose, diet_quality, health_status, recommendation)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (age, gender, body_temperature, pulse_rate, respiration_rate, blood_pressure, blood_oxygen, 
              weight, blood_glucose, diet_quality, result, recommendation))
        conn.commit()
        conn.close()
